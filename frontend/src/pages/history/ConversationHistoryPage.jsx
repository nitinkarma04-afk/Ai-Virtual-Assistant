  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between selection:bg-cyan-500/30 selection:text-cyan-200 relative overflow-x-hidden">
          ...
          <div className="flex items-center gap-3">
            <Link to="/dashboard">
              <Button
                ...
                title="Return to Assistant Dashboard"
              >
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 font-mono mt-0.5">
                  {conversations.length} {conversations.length === 1 ? 'logged session' : 'logged sessions'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {conversations.length > 0 && (
              <Button
                ...
                className="text-slate-400 hover:text-rose-400 text-xs px-3"
              >
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10 relative z-10 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-3xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl shadow-xl">
          <div className="space-y-1">
            ...
            <h2 className="text-2xl font-bold text-white tracking-tight">Past Interaction Logs</h2>
            <p className="text-sm text-slate-400 max-w-xl">
              Inspect past query prompts, assistant answers, and historical context stored in your personal virtual assistant memory.
            </p>
          </div>

          <div className="flex items-center gap-3 shrink-0">
            <Link to="/dashboard">
              <Button variant="glow" size="sm" icon={Sparkles}>
                New Query
              </Button>
            </Link>
          </div>
        ...
        {error && (
          <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/30 flex items-start gap-3 text-rose-300 text-sm">
            ...
            <div className="flex-1">
              <strong className="font-semibold block text-rose-200">Failed to load history</strong>
              <span>{error}</span>
            </div>
            <Button variant="ghost" size="sm" onClick={() => fetchHistory(true)} className="text-rose-300 hover:text-white">
              Retry
            </Button>
          </div>
        )}

        {/* Search & Filter Bar (Shown when history exists or searching) */}
        {(conversations.length > 0 || searchQuery) && (
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search history by keyword in user prompt or assistant response..."
              className="w-full pl-11 pr-4 py-3 rounded-2xl bg-slate-900/80 border border-slate-800 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/30 transition-all"
            />
            {searchQuery && (
              <button
      ...
      <footer className="w-full py-6 px-4 text-center text-xs text-slate-600 font-mono border-t border-slate-900 mt-12 relative z-10">
        Neural Assistant Conversation History Log • {assistantName}
      </footer>
