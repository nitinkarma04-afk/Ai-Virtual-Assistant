...
/**
 * Custom hook to manage AI conversation state, API streaming/dispatch, and visual assistant states.
 */
export const useChat = () => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        if (Array.isArray(rawList) && rawList.length > 0) {
          const formatted = rawList.flatMap((item, idx) => {
            ...
            if (item.message && item.response) {
              const baseTime = item.createdAt || item.timestamp || new Date().toISOString()
              const baseId = item._id || item.id || `hist-${idx}`
              return [
                {
                  id: `${baseId}-usr`,
                  sender: 'user',
            ...
            return [
              {
                ...
                sender: item.sender || (item.role === 'user' ? 'user' : 'assistant'),
                text: item.text || item.content || item.message || item.response || '',
                timestamp: item.timestamp || item.createdAt || new Date().toISOString(),
                actionData: item.action || item.actionData || null,
              },
            ]
          })
        ...
        console.warn('History fetch notice:', err?.message || err)
      }
    }
  ...
  const extractResponseText = (data) => {
    if (!data) return 'I received your request, but no response text was generated.'
  ...
  const sendMessage = async (inputText) => {
    ...
    try {
      ...
      const actionPayload = response?.action || response?.data?.action || null
